...
import MissingTransactionsService from './MissingTransactionsService';
class MissingTransactions extends React.Component {
	constructor(props) {
		super(props);
		this.missingTransactionsService = new MissingTransactionsService();
		this.signInService = new SignInService();
		let defaultFilters = [{ columnField: 'isValidTrans', value: false, operatorValue: 'equals' }, { columnField: 'discrepancyType', value: 1, operatorValue: 'equals' }]
		this.state = { defaultFilters: defaultFilters, redirectTo: null, redirect: false, density: "compact", filterModel: { items: defaultFilters }, sortModel: [{ field: "createdDate", sort: "desc" }], transactions: { count: 0, mpesaReconTransactions: [] }, page: 0, rowsPerPage: 10, loading: true, rowsPerPageOptions: [10, 15, 20, 25, 50, 100] };
	}
	componentDidMount() {
		this.getMissingTransactions(this.state.page, this.state.rowsPerPage, this.state.sortModel, this.state.filterModel);
	}
			...
			if (this.state.redirectTo === "signin") {
				return (
					<Redirect to={{
						...
						state: { fromUrl: config.MissingTransRouteURL }
					}} />
				);
			}
		}
	}
	getMissingTransactions = (page, pageSize, sortModel, filterModel) => {
		try {
			var f = sortModel[0];
			this.missingTransactionsService.retrieveMissingTransactions(page, pageSize, f.field, f.sort, filterModel, this.props.accessToken).then(data => {
				if (data != null) {
					if (data.error != null) {
